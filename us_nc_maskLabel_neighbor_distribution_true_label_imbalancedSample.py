...
from us_lgc_true_label_51by6_neighbor_distribution_in_mem_dataset import USLGCDistributionFeaturesTrueLabelDataset
EPS = 1e-15

# %% 
# Load dataset
dataset = USLGCDistributionFeaturesTrueLabelDataset("./data/")
# examine the graph
...
train_loader = NeighborLoader(data, num_neighbors=[10] * 2, batch_size=32768,
                                ...
                                sampler=sampler)
val_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=64, 
                                input_nodes=data.val_mask)
test_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=64,
                                input_nodes=data.test_mask)

total_labeled_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=64,
...
for epoch in range(1, 301):
    ...
    #     print(f'Train: {train_acc:.4f}, Val: {val_acc:.4f}, Test: {test_acc:.4f}')
    # if epoch % 50 == 0:
    #     total_acc, val_acc, test_acc= test_3_sets()
    #     print(f'Total labeled: {total_acc:.4f}, Val: {val_acc:.4f}, Test: {test_acc:.4f}')
...
# # PATH = './model/maskLabel_true_label_imbalanceSample/maskLabel_true_label_imbalanceSample_2.pt'
# # torch.save({
# #             'epoch': 300,
...
# #             'loss': 0.5155581236,
# #             }, PATH)
# PATH = './model/maskLabel_true_label_imbalanceSample/maskLabel_true_label_imbalanceSample.pt'
...
# optimizer_copy = torch.optim.Adam(model.parameters(), lr=0.001  ,  weight_decay=5e-4)

# checkpoint = torch.load(PATH)
# model_copy.load_state_dict(checkpoint['model_state_dict'])
# optimizer_copy.load_state_dict(checkpoint['optimizer_state_dict'])
