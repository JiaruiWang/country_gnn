...
#define NUM_OF_STATES 45 // for true label only 
...
public:
	...
	Page (int idx, unsigned long id, bool mask, int state_idx
		//   , string state_name
		  ) : 
		new_idx(idx), pageID(id), true_label_mask(mask), state_idx(state_idx),
		// state(state_name),
