...
void Graph::parse_page (ifstream& is) {
	...
	cout << "-----------------start parsing states-------------------" << endl;
	// read in states and states_no_tops to build trueLabel converstion map
	ifstream states_no_tops_file ("../new_city_data/states_no_centers.csv");
	string line_state, state_str;
	int count = 0;
	unordered_map<std::string, int> states_no_tops_name_idx;
	while (states_no_tops_file.good()) {
		getline(states_no_tops_file, line_state);
		stringstream ss(line_state);
		getline(ss, state_str, ',');
		states_no_tops_name_idx[state_str] = count;
		// std::cout << state_str << ": " << count << endl;
		count ++;
	}
	// for (auto const &pair: states_no_tops_name_idx) {
    //     std::cout << "{" << pair.first << ": " << pair.second << "}\n";
    // }
	cout << "-----------------start parsing states-------------------" << endl;
	ifstream states_file("../new_city_data/states.csv");
	count = 0;
	unordered_map<int, std::string> states_idx_name;
	while (states_file.good()) {
		getline(states_file, line_state);
		stringstream ss(line_state);
		getline(ss, state_str, ',');
		states_idx_name[count] = state_str;
		// std::cout << count << ": " << state_str << endl;
		count ++;
	}
	// for (auto const &pair: states_idx_name) {
    //     std::cout << "{" << pair.first << ": " << pair.second << "}\n";
    // }
	// std::cout << 50 << ", " << states_idx_name[50] << ": "// << states_no_tops_name_idx[states_idx_name[50]]
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[50]) == states_no_tops_name_idx.end()) << endl;
	// std::cout << 10 << ", " << states_idx_name[10] << ": "// << states_no_tops_name_idx[states_idx_name[10]] 
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[10]) == states_no_tops_name_idx.end()) << endl;
	// std::cout << 42 << ", " << states_idx_name[42] << ": "// << states_no_tops_name_idx[states_idx_name[42]] 
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[42]) == states_no_tops_name_idx.end()) << endl;
	// std::cout << 31 << ", " << states_idx_name[31] << ": "// << states_no_tops_name_idx[states_idx_name[31]] 
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[31]) == states_no_tops_name_idx.end()) << endl;
	// std::cout << 4 << ", " << states_idx_name[4] << ": "// << states_no_tops_name_idx[states_idx_name[4]] 
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[4]) == states_no_tops_name_idx.end()) << endl;
	// std::cout << 0 << ", " << states_idx_name[0] << ": " //<< states_no_tops_name_idx[states_idx_name[0]] 
	// 	<< ", " << (states_no_tops_name_idx.find(states_idx_name[0]) == states_no_tops_name_idx.end()) << endl;
	...
	int count_washington = 0, count_5200_washington = 0, count_true_label = 0, count_positive_stateid = 0;
	bool mask;
	string city, line;
	...
	while (is.good()) {
		...
		state_idx = trueLabel;
		if (states_no_tops_name_idx.find(states_idx_name[state_idx]) == states_no_tops_name_idx.end()) {
			// std::cout << "tops: " << state_idx;
			state_idx = -1;
			// std::cout << ", " << state_idx << endl;
		} else {
			// std::cout << "no tops: " << state_idx;
			state_idx = states_no_tops_name_idx[states_idx_name[state_idx]];
			// std::cout << ", " << state_idx << endl;
		}
	...
	cout << "count_positive_stateid " << count_positive_stateid << endl;
	cout << "-----------------------------------------------" << endl;
}
...
void Graph::write(string directory) {
	...
	ofstream ofs;
	string file = directory + "new_cities_true_label_no_centers_45_by_6_neighbor_distribution.csv";
