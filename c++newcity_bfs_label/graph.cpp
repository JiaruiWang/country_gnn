...
void Graph::parse_page (ifstream& is) {
	...
	string pageID_str, new_idx_str, dupStates_str, trueLabel_str,
		   firstRoundLabel_str, secondRoundLabel_str;
	while (is.good()) {
		getline(is, line);
		...
		getline(ss, secondRoundLabel_str, ',');
		getline(ss, firstRoundLabel_str, ',');
		...
		//  	 << mostPopulationLabel << " " << state_idx << " " << firstRoundLabel << " is.eof "
		...
		head ../new_city_data/new_cities_2hop_bfs_2_round_id_idx_city_dupStates_trueLabel_2ndRoundLabel_1stRoundLabel.csv
