...
void Graph::parse_page (ifstream& is) {
	...
	bool mask;
	while (is.good()) {
		is >> new_idx >> pageID >> mask >> state_idx;
		getline(is, state, '\n'); // is >> state will only read "New" from "New York".
		if (is.eof()) break;
		cnt++;
		Page * current = new Page(new_idx, pageID, mask, state_idx, state);
		pageMap[new_idx] = current;
		if (cnt % 200000 == 0) {
...
void Graph::count_neighbor (int root, int distance, string direction) { // make sure root != 0, column is column # each page is going to fill.
	...
	while (distance >= dist && !unvisited.empty()) {
		...
		if (direction == "inward") {
			for (size_t i = 0; i < current->inward_neighbor.size(); i++) { // only_in_states anchor pages only have inward edges no outward edges.
...
void Graph::write(string directory) {
	...
	ofstream ofs;
	string file = directory + "us_lgc_51_by_6_neighbor_distribution.csv";
