...
truelabel_file = '../saint_newcity_truelabel_A_to_A/id_label_state_pred_state_threshold_counts_possible_states.csv'
with open(inputfile, 'r') as file, open(full_info, 'r') as fullinfo, open(truelabel_file, 'r') as truelabel_info:
    csvreader = csv.reader(file)
    inforeader = csv.reader(fullinfo, delimiter='\t')
    truelabelreader = csv.reader(truelabel_info)
    for (row, info, truelabelinfo) in zip(csvreader, inforeader, truelabelreader):
...
outputfile = './tuple_keys_contains_pages_population_labeling_fan_order-no_6_centers_A_2147399.csv'
with open(outputfile, 'w') as file:
    csvwriter = csv.writer(file, delimiter='\t')
    for i in range(2, 30):
        # row = [i]
