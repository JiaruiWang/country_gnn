...
class USLGCNewcityTrueLableNo6CentersDistributionFeaturesWithIdDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.data = torch.load(osp.join(self.processed_dir, f'us_lgc_newcity_truelabel_no_6_centers_45by6_neighbor_distribution_with_id_in_mem_dataset.pt'))
    @property
    ...
    def processed_file_names(self):
        ...
        return ['us_lgc_newcity_truelabel_no_6_centers_45by6_neighbor_distribution_with_id_in_mem_dataset.pt']
    ...
    def process(self):
        ...
        us_x = self.get_node_feature('../../new_city_data/new_cities_true_label_no_centers_45_by_6_neighbor_distribution_no_ca_ny_fl_tx_pa_il.csv')
        y_has_centers_51, labeled_mask, y_no_centers_45, labeled_mask_no_centers = self.get_y_label('../../new_city_data/new_cities_2hop_bfs_2_round_pl_id_idx_city_dupStates_trueLabel_mostPopulationLabel_2ndRoundDupstate0_1stRoundDupstate0.csv')
        ...
        self.data = Data(x=us_x, edge_index=us_edge_index, y=y_no_centers_45)
        self.data.num_classes = 45
        self.data.id = us_id
        self.data.y_has_centers_51 = y_has_centers_51
        self.data.labeled_mask_has_centers_51 = labeled_mask
        self.data.labeled_mask_no_centers_45 = labeled_mask_no_centers
        ...
        # print(data)
        # print(slices)
        torch.save(self.data, osp.join(self.processed_dir, f'us_lgc_newcity_truelabel_no_6_centers_45by6_neighbor_distribution_with_id_in_mem_dataset.pt'))
    ...
    def get_y_label(self, file_path: str):
        ...
        for i in range(len(trueLabel)):
            # print(city_df.values[i][0], trueLabel_df.values[i][0])
            if trueLabel[i][0] in [4, 8, 12, 31, 37, 42]:
                trueLabel[i][0] = -1
                count_centers += 1
            if trueLabel[i][0] != -1:
...
# %% test the dataset 
root = "../../data/"
dataset = USLGCNewcityTrueLableNo6CentersDistributionFeaturesWithIdDataset(root)
print(dataset[0])
