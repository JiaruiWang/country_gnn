...
from us_lgc_newcity_truelabel_no_centers_45by6_neighbor_distribution_with_id_in_mem_dataset import USLGCNewcityTrueLableNo6CentersDistributionFeaturesWithIdDataset
EPS = 1e-15

# %% 
# Load dataset
dataset = USLGCNewcityTrueLableNo6CentersDistributionFeaturesWithIdDataset("../../data/")
# examine the graph
...
labeled_indices = data.labeled_mask_no_centers_45.nonzero(as_tuple=False).view(-1)
n = data.labeled_mask_no_centers_45.shape[0]
...
val_indices = labeled_indices[0:133000]
test_indices = labeled_indices[133000:266000]
train_indices = labeled_indices[266000:]
...
print(f'Number of labeled_mask_no_centers_48 nodes: {data.labeled_mask_no_centers_45.sum()}')
...
#%%
subgraph_edge_index, subgraph_edge_weight = subgraph(data.labeled_mask_no_centers_45, data.edge_index, data.edge_weight, relabel_nodes=True)
# print(subgraph_edge_index.shape, subgraph_edge_weight.shape)
subgraph_x = data.x[data.labeled_mask_no_centers_45]
subgraph_y = data.y[data.labeled_mask_no_centers_45]
...
subgraph_data.num_classes = 45
subgraph_data.train_mask = data.train_mask[data.labeled_mask_no_centers_45]
subgraph_data.val_mask = data.val_mask[data.labeled_mask_no_centers_45]
subgraph_data.test_mask = data.test_mask[data.labeled_mask_no_centers_45]
print(subgraph_data)
...
total_labeled_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=16384,
                                input_nodes=data.labeled_mask_no_centers_45)
total_data_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=16384)
...
#%%
# load model
PATH = f'./truelabel_no_6_centers/1/'
file = PATH + 'best_auc_epoch_137_loss_24.2715_testauc_0.8989.pt'
...
@torch.no_grad()
def output_infer(loader, filename:str):
    model.eval()
    torch.cuda.empty_cache()
    ...
    for sampled_data in tqdm.tqdm(loader):
        ...
        out_total.extend(out[:sampled_data.batch_size].cpu().tolist())  
    ...
    for i in range(len(id_total)):
        ...
        row.extend(out_total[i])
        output_list.append(row)
    print('len(output_list)', len(output_list))
    print('len(output_list[0])', len(output_list[0]))


    outdf = pd.DataFrame(output_list)
    print('outdf.shape', outdf.shape)
    print('outdf[0:1]', outdf[0:1])
    outdf.to_csv(filename, sep=',', header=False, index=False)
    return id_total, y_total, pred_total, out_total
# %%
...
# %%
filename = './saint_newcity_truelabel_A_to_A_no_6_centers_45_infer_A_id_y_pred_45probability.csv'
id_total, y_total, pred_total, out_total = output_infer(total_labeled_loader, filename)
...
#%%
filename = './saint_newcity_truelabel_A_to_A_no_6_centers_45_infer_AB_id_y_pred_45probability.csv'
id_total, y_total, pred_total, out_total = output_infer(total_data_loader, filename)
#%%