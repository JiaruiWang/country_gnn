...
from us_lgc_newcity_mnl_more_washington_51by6_neighbor_distribution_with_id_in_mem_dataset import USLGCNewcityMNLMoreWashingtonDistributionFeaturesWithIdDataset
EPS = 1e-15

# %% 
# Load dataset
dataset = USLGCNewcityMNLMoreWashingtonDistributionFeaturesWithIdDataset("../../data/")
# examine the graph
...
data.train_mask = pyg.utils.index_to_mask(train_indices, n)  # all from labeled mask
data.val_mask = pyg.utils.index_to_mask(val_indices, n)  # all from labeled mask
data.test_mask = pyg.utils.index_to_mask(test_indices, n)  # all from labeled mask

data.unlabeled_mask = (data.labeled_mask != 1)
print('data.labeled_mask.sum()', data.labeled_mask.sum())
print('data.unlabeled_mask.sum()', data.unlabeled_mask.sum())
data.train_AB = (data.train_mask + data.unlabeled_mask)
print('data.train_AB.sum()', data.train_AB.sum())
...
print(f'Number of unlabeled_mask nodes: {data.unlabeled_mask.sum()}')
print(f'Number of train_AB nodes: {data.train_AB.sum()}')
