...
from us_lgc_newcity_truelabel_no_centers_48by6_neighbor_distribution_with_id_in_mem_dataset import USLGCNewcityTrueLableNoCentersDistributionFeaturesWithIdDataset
EPS = 1e-15

# %% 
# Load dataset
dataset = USLGCNewcityTrueLableNoCentersDistributionFeaturesWithIdDataset("../../data/")
# examine the graph
...
labeled_indices = data.labeled_mask_no_centers_48.nonzero(as_tuple=False).view(-1)
n = data.labeled_mask_no_centers_48.shape[0]
...
print(f'Number of labeled_mask_no_centers_48 nodes: {data.labeled_mask_no_centers_48.sum()}')
...
#%%
subgraph_edge_index, subgraph_edge_weight = subgraph(data.labeled_mask_no_centers_48, data.edge_index, data.edge_weight, relabel_nodes=True)
# print(subgraph_edge_index.shape, subgraph_edge_weight.shape)
subgraph_x = data.x[data.labeled_mask_no_centers_48]
subgraph_y = data.y[data.labeled_mask_no_centers_48]
...
subgraph_data.num_classes = 48
subgraph_data.train_mask = data.train_mask[data.labeled_mask_no_centers_48]
subgraph_data.val_mask = data.val_mask[data.labeled_mask_no_centers_48]
subgraph_data.test_mask = data.test_mask[data.labeled_mask_no_centers_48]
print(subgraph_data)
...
total_labeled_loader = NeighborLoader(data, num_neighbors=[-1] * 2, batch_size=16384,
                                input_nodes=data.labeled_mask_no_centers_48)
...
#%%
# load model
PATH = f'./truelabel_no_centers/1/'
file = PATH + 'best_auc_epoch_206_loss_26.2658_testauc_0.8927.pt'
...
# %%
filename = './saint_newcity_truelabel_A_to_A_no_centers_48_infer_A_id_y_pred_48probability.csv'
...
#%%
filename = './saint_newcity_truelabel_A_to_A_no_centers_48_infer_AB_id_y_pred_51probability.csv'
