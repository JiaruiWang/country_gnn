...
# %%
knn = KNN(Spectral(n_components=32), n_neighbors=501)
knn_spectral_32_501_labels = knn.fit_transform(directed.adjacency, seeds)
# %%
print(knn_spectral_32_501_labels.shape)
# %%
print(type(knn_spectral_32_501_labels))
# %%
# pd.DataFrame(labels).to_csv('./data/raw_dir/us_lgc_knn_gsvd_8_20.csv')
with open('./data/raw_dir/us_lgc_knn_spectral_32_501_.csv', 'w') as csvfile:
    writer = csv.writer(csvfile)
    for r in knn_spectral_32_501_labels:
        row = str(r.item()) 
        writer.writerow([row])
