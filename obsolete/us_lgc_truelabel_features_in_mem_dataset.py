...
class USLGCTrueLabelFeaturesDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.data = torch.load(osp.join(self.processed_dir, f'us_lgc_true_label_features_in_mem_dataset.pt'))
    @property
    ...
    def processed_file_names(self):
        ...
        return ['us_lgc_true_label_features_in_mem_dataset.pt']
    ...
    def process(self):
        ...
        us_x = self.get_node_feature('./data/raw_dir/us_pages_lgc_true_label_51_label.csv')
        us_y, mask = self.get_y_label('./data/raw_dir/us_pages_lgc_true_label_51_label.csv')
        self.data = Data(x=us_x, edge_index=us_edge_index, y=us_y)
        self.data.num_classes = 51
        self.data.labeled_mask = mask
        ...
        # print(data)
        # print(slices)
        torch.save(self.data, osp.join(self.processed_dir, f'us_lgc_true_label_features_in_mem_dataset.pt'))
    ...
    def get_node_feature(self, file_path: str = None) -> torch.Tensor:
        ...
        true_label = df.iloc[:, 0:1].values
        label_51 = df.iloc[:, 1:2].values
        x = np.zeros((label_51.shape[0], 51), dtype=np.float32)
    ...
    def get_y_label(self, file_path: str) -> Tuple[torch.LongTensor, torch.BoolTensor]:
...
# %% test the dataset 
# root = "./data/"
# dataset = USLGCTrueLabelFeaturesDataset(root)
