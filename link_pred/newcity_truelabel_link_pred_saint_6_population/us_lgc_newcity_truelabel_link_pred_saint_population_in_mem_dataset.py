...
class USLGCNewcityTruelabelPopulationDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.data = torch.load(osp.join(self.processed_dir, f'us_lgc_newcity_truelabel_link_pred_saint_population_in_mem_dataset.pt'))
    @property
    ...
    def processed_file_names(self):
        ...
        return ['us_lgc_newcity_truelabel_link_pred_saint_population_in_mem_dataset.pt']
    ...
    def process(self):
        ...
        torch.save(self.data, osp.join(self.processed_dir, f'us_lgc_newcity_truelabel_link_pred_saint_population_in_mem_dataset.pt'))
    ...
    def get_node_feature(self, file_path: str = None) -> torch.Tensor:
        ...
        # city population
        population_file = '../../probability_analysis/id_city_cityPopulation_county_countyFips.csv'
        population_list = []
        with open(population_file, 'r') as file:
            csvreader = csv.reader(file, delimiter='\t')
            for row in csvreader:
                population_list.append([int(row[2])])
        p_tensor = torch.Tensor(population_list)
        print(p_tensor)
        p_min, p_max = p_tensor.min(), p_tensor.max()
        p_tensor_norm = (p_tensor - p_min) / (p_max - p_min)
        x = p_tensor_norm
        print(x.shape)
        print(x[0:10])
        return x
...
# %% test the dataset 
root = "../../data/"
dataset = USLGCNewcityTruelabelPopulationDataset(root)
# %%
