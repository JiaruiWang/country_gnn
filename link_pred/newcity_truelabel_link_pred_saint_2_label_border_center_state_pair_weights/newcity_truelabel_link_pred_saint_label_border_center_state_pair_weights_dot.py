...
from torch_geometric.utils import negative_sampling, degree, subgraph
from torch_geometric.loader import LinkNeighborLoader, GraphSAINTRandomWalkSampler

from us_lgc_newcity_truelabel_link_pred_saint_label_border_center_state_pair_weights_in_mem_dataset import USLGCNewcityTruelabelLableBorderCenterStatePairWeightsDataset
...
transform = T.Compose([
    ...
    # T.ToDevice(device),
    T.RandomLinkSplit(num_val=0.002, num_test=0.002, is_undirected=False,
                    #   disjoint_train_ratio=0.3,
                      add_negative_train_samples=False, neg_sampling_ratio=5),
])
# %% 
# Load dataset
dataset_all = USLGCNewcityTruelabelLableBorderCenterStatePairWeightsDataset("../../data")
print(dataset_all)
print(type(dataset_all))

#%%
dataset = USLGCNewcityTruelabelLableBorderCenterStatePairWeightsDataset("../../data/", transform=transform)
# examine the graph
...
#%%
train_data, val_data, test_data = transform(subgraph_data)
...
for epoch in range(start, end):
    ...
    if best_loss > loss:
        best_loss = loss
    model_list.append( deepcopy(net.state_dict()))
    if epoch % 1 == 0:
        ...
        print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Test: {test_auc:.4f}, Time: {minute:03d} min {sec:02d} sec')

# %%
# Save the model
PATH = f'./dot_pts/test_neg_5/'
...
file = PATH + f'best_auc_epoch_{start + max_index:03d}_loss_{loss_list[max_index]:.4f}_testauc_{test_auc_list[max_index]:.4f}.pt'
print(file)
